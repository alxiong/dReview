...
function getStoreFromUrl(url){
	if (url.match("https://www.google.com.sg/maps/place/")){
		...
		var storeName = results[5].split('+').join(' ');
		...
		if (lastIndex > 0){
			var storeId = results[5].split('+').join('') + storeLatLng[storeLatLng.length - 2].slice(2) + storeLatLng[storeLatLng.length - 1].slice(2, lastIndex);
		} else{
			var storeId = results[5].split('+').join('') + storeLatLng[storeLatLng.length - 2].slice(2) + storeLatLng[storeLatLng.length - 1].slice(2);
		}
		return true;
	} else {
		return false;
